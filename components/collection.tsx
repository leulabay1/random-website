    ...
    <div className="mt-[48px] lg:flex lg:flex-row">
      <div className='lg:w-[240px] lg:flex lg:flex-col'>
        <div className='lg:flex-1'>
          <div className='font-semibold text-[18px] leading-[24px] text-[#333333]'>{collection.title}</div>
          <div className='mt-[8px] text-[12px] font-medium leding-[20px] text-[#999999]'>{collection.subtitle}</div>
        </div>
        <div className='hidden lg:block'>
          <button className='mr-[10px]' onClick={() => sliderRef.current?.slidePrev()}>
            <svg
      ...
      <div className='grid grid-cols-2 gap-y-[24px] gap-x-[8px] lg:hidden'>
        {collection.items.map((item: any, key: number) => <CollectionProduct key={key} product={item}/>)}
      </div>
      <div>
